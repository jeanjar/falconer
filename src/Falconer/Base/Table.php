...
class Table
{
    ...
    public function render($caption, $lista, $pager = null, array $options = array(), array $formatters = array(), array $provider = array(), array $skip = array(), $indexCheck = null, $indexTrClass = null, array $subTable = array())
    {
        ...
        foreach ($sample as $key => $ignore_me)
        {
            ...
            $thead .= '<th class="' . $key . '">' . Core::label($key) . '</th>';
        }
        if (!empty($options))
        {
        ...
        $class = [
            'table', 'table-nomargin', 'table-bordered', 'dataTable', 'table-striped', 'table-hover'
        ];
        ...
        ob_start();
        include $this->defaultTemplate;
        return ob_get_clean();
        return '<table class="' . implode(' ', $class) . '">' . $options_massive_html . '<caption>' . $caption . '</caption>' . $thead . $tfoot . $tbody . '</table>';
    }
    ...
    public static function renderHorizontal($caption, $lista, array $formatters = array(), array $provider = array(), array $skip = array(), array $class = array())
    {
        ...
        foreach ($lista as $key => $value)
        {
            ...
            $tbody .= '<th>' . Core::label($key) . '</th>' . $cell;
            $tbody .= '</tr>';
        }
        ...
        return '<table class="' . implode(' ', $class) . '"><caption class="label">' . $caption . '</caption>' . $tbody . '</table>';
    }
